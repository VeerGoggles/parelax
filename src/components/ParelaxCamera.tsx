...
import { Props } from "@zappar/zappar-react-three-fiber/lib/spec";
import useStore from "@zappar/zappar-react-three-fiber/lib/store";
 ...
 */
const ParelaxCamera = forwardRef((props: Props.Camera, ref) => {
  const {
    userFacing = false,
  ...
  useEffect(() => {
    ...
    switch (rearCameraMirrorMode) {
      case "poses":
        cameraRef.current.rearCameraMirrorMode =
          ZapparThree.CameraMirrorMode.Poses;
        break;
      case "css":
        cameraRef.current.rearCameraMirrorMode =
          ZapparThree.CameraMirrorMode.CSS;
        break;
      case "none":
      default:
        cameraRef.current.rearCameraMirrorMode =
          ZapparThree.CameraMirrorMode.None;
        break;
    }
    switch (userCameraMirrorMode) {
      case "none":
        cameraRef.current.userCameraMirrorMode =
          ZapparThree.CameraMirrorMode.None;
        break;
      case "css":
        cameraRef.current.userCameraMirrorMode =
  ...
  useFrame(({ gl, scene }) => {
    ...
    if (
      onFirstFrame &&
      !hadFirstFrame.current &&
      cameraRef.current.pipeline.frameNumber() > 0
    ) {
